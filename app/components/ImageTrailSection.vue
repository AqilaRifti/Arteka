<template>
  <div class="flex flex-col gap-2 w-full min-h-96">
    <div
      class="w-full border mt-4 rounded-lg p-4 flex items-center justify-center h-96 relative"
    >
      <div class="absolute">
        <span class="inset-0 flex items-center justify-center text-2xl text-black dark:text-white"
          >Your Art On</span
        >
        <TextHighlight class="bg-gradient-to-r from-yellow-400 to-yellow-500">
          <span
            class="inset-0 flex items-center justify-center text-4xl text-white"
          >
            <LineShadowText class="italic" :shadow-color="shadowColor">
              BlockChain</LineShadowText
            ></span
          >
        </TextHighlight>
      </div>
...
import type { VariantType } from "../../ui/image-trail-cursor/ImageTrailCursor.vue";
...
const selectedVariant = ref<VariantType>("type2");
</script>
